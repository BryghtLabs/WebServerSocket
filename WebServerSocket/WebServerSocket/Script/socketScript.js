...
try {
    ...
    socket.onopen = function(e) {
        try {
            ...
            console.error('An open-socket error occurred:', error);
    ...
    socket.onmessage = function(event) {
        try {
            ...
            const errorEntity = { msgType: 5, msgMonitor: { isError: true, errMsg: "Socket On Message Error: " + error.message, msg: "" } }
    ...
    socket.onclose = function(event) {
        try {
            ...
            const errorEntity = { msgType: 7, msgMonitor: { isError: true, errMsg: "Socket On Close Error: " + error.message, msg: "" } }
    ...
    socket.onerror = function(error) {
        try {
            ...
            alert(`[error]`);
            console.error('An on-main-error-socket error occurred:', error);
            const errorEntity = { msgType: 8, msgMonitor: { isError: true, errMsg: "Socket On Main Error: " + JSON.stringify(error) + ` code = ${error.code}` + ` reason = ${error.reason}`, msg: "" } }
            ...
            const errorEntity = { msgType: 8, msgMonitor: { isError: true, errMsg: "Socket On Main Error: " + JSON.stringify(errors) + ` code = ${error.code}` + ` reason = ${error.reason}`, msg: "" } }
            window.webkit.messageHandlers.socketMainError.postMessage(errorEntity);
            window.console.log = captureLog;
        }
